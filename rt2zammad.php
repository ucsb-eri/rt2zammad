...
class rt2zammad {
	...
	function createSingleTicket($ticketId = 0){
		...
		while($transaction=mysqli_fetch_assoc($result1)){
			...
			myErrorLog("##-------- Transaction {$transaction['id']} ({$transaction['Type']}) related to Ticket $ticketId$txStatus (Orig: {$transaction['OrigTicketId']}) --------##");
			...
			myErrorLog("transaction['Requestor']:".$transaction['Requestor']);
			...
			if (isset($logcopy['attachments'])){
				foreach($logcopy['attachments'] as &$att){
